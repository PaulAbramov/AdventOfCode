...
for (int i = 0; i < lines.Count; i++)
{
    for (int j = 0; j < lines[0].Length; j++)
    {
        var result = TreeVisibilityChecker.GetTreeScenicScore(i, j, grid);
        if (result > topScenicScore)
        {
            topScenicScore = result;
        }
    }
}

Console.WriteLine($"Top scenic score is {topScenicScore}");