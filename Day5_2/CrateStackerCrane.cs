...
public class CrateStackerCrane
{
    ...
    public void MoveCrates(int amount, int fromStack, int toStack)
    {
        ...
        {
            if (crates[fromStack - 1].Count > 0)
            {
                tempStack.Push(crates[fromStack - 1].Pop());
            }
        }

        while (tempStack.Count > 0)
        {
            crates[toStack - 1].Push(tempStack.Pop());
        }
    }
    ...
    {
        return crates.Aggregate(string.Empty, (current, cratesStack) =>
        {
            if (cratesStack.Count > 0)
            {
                return current + cratesStack.Peek();
            }
