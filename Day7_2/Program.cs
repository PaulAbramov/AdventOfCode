...
{
    manager.CheckInput(line);
}

var folderToDelete = manager.GetSmallestFolderToFreeUpEnoughSpace();

Console.WriteLine($"Folder '{folderToDelete.Name}' would be the smallest folder to free up enough space for the update with a size of {folderToDelete.Size}");